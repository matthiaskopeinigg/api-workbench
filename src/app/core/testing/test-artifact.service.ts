...
export class TestArtifactService {
  ...
  private testArtifactDeletedSubject = new Subject<{ kind: TestingArtifactKind; id: string }>();
  ...
  async remove(kind: TestingArtifactKind, id: string): Promise<void> {
    const had = this.streams[kind].value.some((a) => a.id === id);
    if (!had) {
      return;
    }
    ...
    this.zoneEmit(kind, list);
    await this.flush(kind);
    if (this.isEditorTabbedTestArtifactKind(kind)) {
      this.zone.run(() => this.testArtifactDeletedSubject.next({ kind, id }));
    }
  }
